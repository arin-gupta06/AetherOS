...
export default function Sidebar() {
  ...
  /** Callback wired to AzureInfrastructurePanel — adds template nodes+edges */
  const handleAzureTemplateLoaded = (template) => {
    if (template?.nodes?.length) addNodes(template.nodes);
  ...
  return (
    <div className="flex h-full shrink-0 mt-3 ml-2 mb-2 bg-transparent gap-2 pointer-events-none">
      {/* Icon rail */}
      <div className="w-14 glass-panel border border-aether-border flex flex-col items-center py-3 gap-2 pointer-events-auto shadow-2xl overflow-y-auto overflow-x-hidden">
        {tabs.map(tab => (
          <button
            key={tab.id}
